...
/**
 * Items for ONE continuous study session — the lesson grown to span what used to
 * be several small lessons, so the learner can page/skip across it:
 *   1. ALL due review cards (UNCAPPED), earliest dueLesson first    -> 'quiz'
 *   2. up to `newCap` brand-new items (FENCED to keep new-item pacing
 *      bite-sized so a cold start isn't a whole-deck marathon)      -> 'intro'
 *   3. fallback: nothing due, nothing new -> lowest-box reviews, capped at newCap.
 * Cold start => `newCap` intro cards (same as a classic small lesson). A returning
 * learner with many cards due gets a long session where skip-ahead pays off.
 */
export function selectSessionItems(
  ...
  // 1. Every due review, earliest first. No cap — this is what skip-ahead is for.
  const due = order
    .filter((k) => introduced(k) && isDue(cardOf(k), upcoming))
    .sort((a, b) => cardOf(a).dueLesson - cardOf(b).dueLesson)
  for (const k of due) items.push({ hangul: k, mode: 'quiz' })

  // 2. New items, fenced to newCap so first-time learning stays paced.
  let added = 0
  for (const k of order) {
    if (added >= newCap) break
    if (!introduced(k)) {
      items.push({ hangul: k, mode: 'intro' })
      added++
    }
  }

  // 3. Nothing due and nothing new -> review the weakest, capped.
  if (items.length === 0) {
    const byBox = order.filter(introduced).sort((a, b) => cardOf(a).box - cardOf(b).box)
    for (const k of byBox) {
